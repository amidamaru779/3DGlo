const sendForm = ({
    ...
    const validateInput = (input) => {
        ...
        if (name === 'user_phone') {
            ...
            // Разрешаются кириллица, пробелы, цифры и знаки препинания
            return /^[А-Яа-яЁё0-9\.,!?;:\s]*$/.test(value);
        }
        return true; // Если поле не связано с валидацией, считаем валидным
    };
    ...
    const validate = (inputs) => {
        ...
        inputs.forEach((input) => {
            if (!validateInput(input)) {
                ...
                input.classList.add('error'); // Добавляем класс ошибки для визуального отображения
            } else {
